...
export async function runAutomation(
  ...
  // Otimização Bolt: se a automação já foi carregada (com user), evita query redundante.
  const automation =
    options.automation ||
...
export async function runDueSchedules(now: Date = new Date()): Promise<{ ran: number; ids: string[] }> {
  // Otimização Bolt: carrega user junto para evitar N+1 no loop do runAutomation
